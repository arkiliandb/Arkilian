...
static int get_env_bool_default(const char *env_var, int default_val) {
  ...
  // strtol (rather than atoi) avoids bugprone-unchecked-string-to-number-
  // conversion and yields 0 (falsy) for malformed input — the documented
  // behavior for an unparseable env var that isn't an explicit "true/yes/1".
  char *end = NULL;
  errno = 0;
  long parsed = strtol(val, &end, 10);
