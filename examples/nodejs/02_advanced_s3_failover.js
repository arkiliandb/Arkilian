/**
 * Arkilian Node.js SDK - Advanced S3 Replication & Disaster Recovery
 * 
 * Demonstrates:
...
const S3_CONFIG = {
    ...
    prefix: process.env.ARKILIAN_S3_PREFIX || 'nodejs-demo'
};
...
async function main() {
    console.log('=== Arkilian Node.js SDK: Advanced S3 Streaming & Cold-Start Hydration ===\n');
    ...
    const [primaryCount] = db.all('SELECT COUNT(*) as count FROM ledger');
    ...
    // Disable backup on the recovered reader node for verification
    process.env.ARKILIAN_ENABLE_BACKUP = '0';
    const recoveredDb = new Arkilian(RESTORED_DB);

    const [recoveredCount] = recoveredDb.all('SELECT COUNT(*) as count FROM ledger');
    ...
    const sampleRows = recoveredDb.all('SELECT entry_id, account_id, delta, note FROM ledger LIMIT 5');
