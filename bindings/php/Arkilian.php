...
class Arkilian {
    ...
    public static function hydrateS3(string $localDbPath, string $dbId, array $s3Config): void {
        $ffi = self::getFFI();

        $cLocal = $localDbPath;
        $cDbId = $dbId;

        $s3 = $ffi->new('arkilian_s3_config');
        $s3->endpoint = $s3Config['endpoint'] ?? '';
        $s3->region = $s3Config['region'] ?? 'us-east-1';
        $s3->bucket = $s3Config['bucket'] ?? '';
        $s3->access_key_id = $s3Config['access_key_id'] ?? ($s3Config['accessKey'] ?? '');
        $s3->secret_access_key = $s3Config['secret_access_key'] ?? ($s3Config['secretKey'] ?? '');
        $s3->session_token = $s3Config['session_token'] ?? null;
        $s3->use_ssl = !empty($s3Config['use_ssl']) ? 1 : 0;
        $s3->timeout_ms = $s3Config['timeout_ms'] ?? 10000;

        $errBuf = $ffi->new('char[1024]');
        $rc = $ffi->arkilian_hydrate_s3($cLocal, $cDbId, \FFI::addr($s3), $errBuf, 1024);

        if ($rc !== HYDRATION_OK) {
            $msg = is_string($errBuf) ? $errBuf : \FFI::string($errBuf);
            throw new RuntimeException("Cold hydration failed (code {$rc}): {$msg}");
        }
    }
